...
import {
  FiBookOpen, FiChevronLeft, FiChevronRight, FiSearch,
  FiChevronDown, FiCode, FiCpu, FiBarChart2, FiCloud,
  FiShield, FiAward, FiUsers, FiZap, FiGlobe,
} from "react-icons/fi";
...
const PARENTS = [
  { label: "Software Learning", slug: "software-learning", short: "SL" },
  { label: "Competitive Exam", slug: "competitive-exam", short: "CE" },
];
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-10">
      <button onClick={() => onPage(page - 1)} disabled={page <= 1}
        className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:text-brand-accent hover:border-brand-accent disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer"
        aria-label="Previous page"
      ><FiChevronLeft className="w-4 h-4" /></button>
      {Array.from({ length: last }, (_, i) => i + 1).map((p) => (
        <button key={p} onClick={() => onPage(p)}
          className={`w-9 h-9 rounded-lg text-sm font-medium transition-all cursor-pointer ${
            p === page ? "bg-brand-accent text-white shadow-sm" : "border border-gray-200 text-gray-600 hover:border-brand-accent hover:text-brand-accent"
...
export default function Courses() {
  ...
  const selectParent = useCallback((slug) => {
    setSearch("");
    setPage(1);
  ...
  const { data: courses, isLoading } = useQuery({
    queryKey: ["allCourses"],
    queryFn: async () => {
      const { data } = await supabase.from("courses").select("*, nav_item_id").eq("is_published", true).order("created_at", { ascending: false });
      return data || [];
    },
  });
  ...
  const { data: navItems } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("nav_items").select("id, label, path, parent_label, parent_id").eq("is_active", true).order("sort_order");
      return data || [];
    },
  });
  ...
  const courseMap = useMemo(() => {
    ...
    const byNav = {};
    navItems.forEach((ni) => { byNav[ni.id] = map[ni.id] || []; });
    return byNav;
  }, [courses, navItems]);
  ...
  const tree = useMemo(() => {
    if (!navItems) return { sl: [], ce: [] };
    const byParent = { "Software Learning": [], "Competitive Exam": [] };
    navItems.forEach((ni) => {
      if (ni.parent_label && byParent[ni.parent_label]) {
        byParent[ni.parent_label].push(ni);
      }
    });
    const expand = (items) => items.map((p) => {
    ...
    return { sl: expand(byParent["Software Learning"]), ce: expand(byParent["Competitive Exam"]) };
  }, [navItems, courseMap]);

  const currentTree = parentParam === "software-learning" ? tree.sl : parentParam === "competitive-exam" ? tree.ce : [];

  const activeNavId = useMemo(() => {
    if (!activeCategory || !navItems) return null;
    const found = navItems.find((ni) => {
      const slug = ni.path ? ni.path.replace(/.*\//, "") : ni.label.toLowerCase().replace(/\s+/g, "-");
  ...
  const shouldExpand = useCallback((parentNode) => {
  ...
  const searchedCourses = useMemo(() => {
    ...
    return filteredCourses.filter((c) => (c.title || "").toLowerCase().includes(q) || (c.slug || "").toLowerCase().includes(q));
  ...
  const sidebarNode = (parentNode) => {
    ...
    const parentSlug = parentNode.path ? parentNode.path.replace(/.*\//, "") : parentNode.label.toLowerCase().replace(/\s+/g, "-");
    ...
    return (
      <div key={parentNode.id}>
        <button
          onClick={() => { if (hasChildren) toggleParent(parentNode.id); selectCategory(parentSlug); }}
        ...
        >
          <span className="flex items-center gap-3 min-w-0 flex-1">
            <span className="w-[18px] h-[18px] flex items-center justify-center shrink-0">
              <Icon className={`w-[18px] h-[18px] ${isParentActive ? "text-orange-600" : "text-gray-400"}`} />
            </span>
            <span className="truncate min-w-0 max-w-full">{parentNode.label}</span>
          </span>
          <span className="flex items-center gap-1.5 shrink-0">
            <span className="text-xs font-medium text-gray-400 tabular-nums leading-none">{parentNode.totalCount}</span>
            {hasChildren && (
              <FiChevronRight
            ...
            {hasChildren && (
              <div className="ml-[22px] border-l-2 border-gray-100 pl-4">
                {parentNode.children.map((child) => {
                  ...
                  const childSlug = child.path ? child.path.replace(/.*\//, "") : child.label.toLowerCase().replace(/\s+/g, "-");
                  const isChildActive = activeCategory === childSlug;
                  return (
                    <button key={child.id} onClick={() => selectCategory(childSlug)}
                    ...
                    >
                      <span className="flex items-center gap-2.5 min-w-0 flex-1">
                        <span className="w-[14px] h-[14px] flex items-center justify-center shrink-0">
                          <ChildIcon className={`w-[14px] h-[14px] ${isChildActive ? "text-orange-600" : "text-gray-400"}`} />
                        </span>
                        <span className="truncate min-w-0 max-w-full">{child.label}</span>
                      </span>
                      <span className="text-xs font-medium text-gray-400 tabular-nums leading-none shrink-0">{countFor(child.id)}</span>
                    </button>
                  );
                })}
              </div>
      ...
      {!parentParam ? (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="text-center mb-10">
            <h1 className="text-[clamp(1.75rem,3vw,2.5rem)] font-extrabold text-dark-navy">Our Courses</h1>
            <p className="text-text-gray text-base sm:text-lg mt-3">Choose a category to explore courses</p>
          </Reveal>
          <div className="grid sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
            {PARENTS.map((p) => (
              <button key={p.slug} onClick={() => selectParent(p.slug)}
                className="bg-white rounded-2xl border-2 border-gray-200 p-8 text-center hover:border-brand-accent hover:shadow-md transition-all group cursor-pointer"
              >
        ...
        <div className="flex w-full">
          {/* Left Sidebar — flush against viewport left edge */}
          <aside className="w-[270px] shrink-0 hidden lg:block pl-4 sm:pl-6 lg:pl-8" aria-label="Course categories">
            <nav className="sticky top-24 py-4 pr-3">
              {/* Segmented control for SL/CE */}
              <div className="flex p-0.5 bg-gray-100 rounded-xl mb-6 mx-3" role="tablist">
                {PARENTS.map((p) => (
                  <button key={p.slug} role="tab" aria-selected={parentParam === p.slug}
          ...
          {/* Right content — wraps both mobile dropdown and course grid */}
          <div className="flex-1 min-w-0 max-w-[1600px]">
            {/* Mobile sidebar (dropdown) */}
            <div className="lg:hidden px-4 sm:px-6 mb-6">
              <div className="flex p-0.5 bg-gray-100 rounded-xl mb-4" role="tablist">
                {PARENTS.map((p) => (
                  <button key={p.slug} role="tab" aria-selected={parentParam === p.slug}
                    onClick={() => selectParent(p.slug)}
                    className={`flex-1 px-3 py-2 text-xs font-bold rounded-[10px] transition-all duration-150 cursor-pointer ${
                      parentParam === p.slug ? "bg-white text-brand-accent shadow-sm" : "text-gray-500 hover:text-gray-700"
                    }`}
                  >{p.short}</button>
                ))}
              </div>
              <div className="relative">
                <button
                  ...
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm font-medium text-dark-navy bg-white focus:outline-none focus:ring-2 focus:ring-brand-accent cursor-pointer flex items-center justify-between gap-2"
                  aria-label="Select a course category"
                  aria-expanded={mobileOpen}
                >
                  <span className="truncate">
                    {activeNavId
                      ? navItems?.find((n) => n.id === activeNavId)?.label || "All Courses"
                      : "All Courses"}
                  </span>
                  <FiChevronDown className={`w-4 h-4 text-gray-400 shrink-0 transition-transform duration-200 ${mobileOpen ? "rotate-180" : ""}`} />
                </button>
                {mobileOpen && (
                  <>
                    ...
                    <div className="absolute left-0 right-0 top-full mt-1 z-20 bg-white border border-gray-200 rounded-xl max-h-[60vh] overflow-y-auto">
                      <button
                        onClick={() => { clearCategory(); setMobileOpen(false); }}
                      ...
                      {currentTree.map((parent) => {
                        const parentSlug = parent.path ? parent.path.replace(/.*\//, "") : parent.label.toLowerCase().replace(/\s+/g, "-");
                        const isParentActive = activeCategory === parentSlug;
                        return (
                          <div key={parent.id}>
                            <button
                              onClick={() => { selectCategory(parentSlug); setMobileOpen(false); }}
                              className={`w-full text-left px-4 py-2.5 text-sm transition-colors cursor-pointer flex items-center justify-between gap-2 overflow-hidden ${
                                isParentActive ? "text-orange-600 font-semibold" : "text-gray-700 hover:text-orange-600 hover:font-semibold"
                              }`}
                            >
                              <span className="flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">{parent.label}</span>
                              <span className="text-xs font-medium text-gray-400 tabular-nums shrink-0">{parent.totalCount}</span>
                            </button>
                            {parent.children.map((child) => {
                              ...
                              return (
                                <button key={child.id}
                                  onClick={() => { selectCategory(childSlug); setMobileOpen(false); }}
                                  className={`w-full text-left pl-8 pr-4 py-2 text-sm transition-colors cursor-pointer flex items-center justify-between gap-2 overflow-hidden ${
                                    isChildActive ? "text-orange-600 font-semibold" : "text-gray-700 hover:text-orange-600 hover:font-semibold"
                                  }`}
                                >
                                  <span className="flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">{child.label}</span>
                                  <span className="text-xs font-medium text-gray-400 tabular-nums shrink-0">{countFor(child.id)}</span>
                                </button>
                              );
                            })}
                          </div>
            ...
            <div className="pr-4 sm:pr-6 lg:pr-8 pl-4 sm:pl-6 lg:pl-10">
              <div className="flex items-center justify-between mb-5 sm:mb-6 flex-wrap gap-3">
                <h1 className="text-lg sm:text-2xl font-bold text-dark-navy">
                  {activeNavId
                    ? navItems?.find((n) => n.id === activeNavId)?.label || "Courses"
                    : PARENTS.find((p) => p.slug === parentParam)?.label || "Courses"}
                </h1>
                <div className="relative w-full sm:w-56 lg:w-60">
                  <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                  <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
                    placeholder="Search courses..."
                    className="w-full pl-10 pr-4 py-2 rounded-xl border border-gray-200 text-sm text-dark-navy bg-white focus:outline-none focus:ring-2 focus:ring-brand-accent transition-all"
              ...
              {searchedCourses.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-text-gray">{search ? `No courses match "${search}"` : "No courses found in this category."}</p>
                </div>
              ) : (
                <>
