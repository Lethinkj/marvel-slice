...
export default function TrainingDetail() {
  ...
  const { data: training, isLoading } = useTrainingProgram(slug);
  const { data: related } = usePublishedTraining({
    limit: 4,
    not_id: training?.id,
  ...
  return (
    <div>
              ...
              <div className="flex flex-wrap items-center gap-3 text-white/70 text-sm mb-6">
                ...
                {training.mode && (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/15 text-white">
                    <ModeIcon className="w-3.5 h-3.5" /> {training.mode}
                  </span>
                )}
                {training.difficulty && (
      ...
      {/* ============ 10. GALLERY ============ */}
      <GallerySection images={training.training_gallery} />
      ...
      {/* ============ 13. STATISTICS ============ */}
      {training.training_statistics?.length > 0 && (
                    ...
                    <div className="p-4">
                      ...
                      {rp.duration && (
                        <span className="inline-flex items-center gap-1 text-xs text-gray-400 mt-2">
                          <FiClock className="w-3 h-3" /> {rp.duration}
                        </span>
                      )}
                    </div>
                  </Link>
...
function TrainingModuleAccordion({ index, module, topics, outcomes }) {
