  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 border border-admin-200" role="dialog" aria-modal="true">
        ...
        <h3 className="text-base font-semibold text-admin-900 mb-1">{title}</h3>
        <p className="text-sm text-admin-600 mb-6">{message}</p>
        <div className="flex justify-end gap-3">
          ...
          <button
            ref={confirmRef}
            onClick={onConfirm}
            className={`px-4 py-2 rounded-lg text-sm font-medium text-white transition-all shadow-sm ${destructive ? "bg-rose-600 hover:bg-rose-700" : "bg-admin-600 hover:bg-admin-700"}`}
          >
            {confirmLabel}
          </button>
        </div>
