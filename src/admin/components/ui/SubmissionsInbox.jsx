    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm cursor-pointer" onClick={onClose}>
      <div className="bg-white rounded-xl border border-admin-200 shadow-2xl max-w-lg w-full cursor-pointer" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-admin-100">
          <div><h3 className="text-base font-semibold text-admin-900">Reply to {row.full_name || row.name || row.email}</h3><p className="text-xs text-admin-400 mt-0.5">{row.email}</p></div>
          <button onClick={onClose} className="p-1.5 text-admin-400 hover:text-admin-600 hover:bg-admin-100 rounded-lg transition-all"><FiX className="w-5 h-5" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-admin-600 mb-1">Subject</label>
            <input type="text" value={subject} onChange={e => setSubject(e.target.value)} placeholder="Subject"
              className="w-full h-10 px-3 rounded-lg border border-admin-200 bg-white text-sm text-admin-900 placeholder:text-admin-400 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-admin-600 mb-1">Message</label>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 cursor-pointer" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-xl border border-admin-200 shadow-2xl w-full max-w-md cursor-pointer" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-admin-100">
          <div><h2 className="text-base font-semibold text-admin-900">Export {type.toUpperCase()}</h2><p className="text-xs text-admin-400 mt-0.5">Choose date range for the report</p></div>
          <button onClick={onClose} className="p-1.5 text-admin-400 hover:text-admin-600 hover:bg-admin-100 rounded-lg transition-all"><FiX className="w-5 h-5" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-admin-600 mb-1">Date Range</label>
            <select value={dateFilter} onChange={e => { setDateFilter(e.target.value); setCustomStart(''); setCustomEnd(''); }}
              className="w-full h-9 px-3 rounded-lg border border-admin-200 bg-white text-sm text-admin-700 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all"
            >
          ...
          {dateFilter === 'custom' && (
            <div className="flex gap-3">
              <div className="flex-1"><label className="block text-xs font-medium text-admin-600 mb-1">From</label>
                <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)}
                  className="w-full h-9 px-3 rounded-lg border border-admin-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all" /></div>
              <div className="flex-1"><label className="block text-xs font-medium text-admin-600 mb-1">To</label>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-admin-100 bg-white/50">
          <span className="text-xs text-admin-400">{filtered.length} entries</span>
...
function DetailRow({ label, value }) {
  ...
  return (
    <div className="space-y-0.5">
      <p className="text-[11px] font-semibold text-admin-400 uppercase tracking-wider">{label}</p>
      <p className="text-sm text-admin-700 whitespace-pre-wrap break-words">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-admin-900">{title}</h1>
          <p className="text-sm text-admin-400 mt-0.5">{data.length} total{unreadCount > 0 ? ` · ${unreadCount} unread` : ''}</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setExportModal('csv')} disabled={filtered.length === 0}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-admin-200 text-xs font-medium text-admin-600 hover:text-admin-800 hover:bg-white transition-all disabled:opacity-40 bg-white shadow-sm"
          >
      ...
      <Card>
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[200px]">
            <label className="block text-[11px] font-medium text-admin-500 mb-1">Search</label>
            <div className="relative">
              <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-admin-400 pointer-events-none" />
              <input ref={searchRef} type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
                placeholder={`Search ${title.toLowerCase()}...`}
                className="w-full pl-9 pr-3 h-9 rounded-lg border border-admin-200 bg-white text-sm text-admin-900 placeholder:text-admin-400 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all"
              />
            </div>
          </div>

          {!disableReply && <div>
            <label className="block text-[11px] font-medium text-admin-500 mb-1">Status</label>
            <div className="relative">
              <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setPage(1); }}
                className="h-9 px-3 pr-8 rounded-lg border border-admin-200 bg-white text-sm text-admin-700 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all appearance-none w-full"
              >
                <option value="all">All</option>
                <option value="unread">Unread</option>
                <option value="read">Read</option>
              </select>
              <FiChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-admin-400 pointer-events-none" />
            </div>
          </div>}

          {extraFilters && extraFilters.map((f, i) => (
            <div key={i}>
              <label className="block text-[11px] font-medium text-admin-500 mb-1">{f.label}</label>
              <div className="relative">
                <select value={f.value} onChange={e => { f.onChange(e.target.value); setPage(1); }}
                  className="h-9 px-3 pr-8 rounded-lg border border-admin-200 bg-white text-sm text-admin-700 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all appearance-none"
                >
          ...
          <div>
            <label className="block text-[11px] font-medium text-admin-500 mb-1">Date</label>
            <div className="relative">
          ...
          {dateFilter === 'custom' && (
            <>
              <div>
                <label className="block text-[11px] font-medium text-admin-500 mb-1">From</label>
                <input type="date" value={customStart} onChange={e => setCustomStart(e.target.value)}
                  className="h-9 px-3 rounded-lg border border-admin-200 bg-white text-sm text-admin-700 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all"
                />
              </div>
              <div>
                <label className="block text-[11px] font-medium text-admin-500 mb-1">To</label>
                <input type="date" value={customEnd} onChange={e => setCustomEnd(e.target.value)}
                  className="h-9 px-3 rounded-lg border border-admin-200 bg-white text-sm text-admin-700 focus:outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all"
                />
              </div>
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="h-9 px-3 text-xs font-medium text-admin-500 hover:text-admin-700 transition-colors">
              Clear filters
            </button>
          )}
        </div>
        ...
        <div className={`${selected ? 'xl:col-span-2' : 'xl:col-span-3'}`}>
          <div className="bg-white rounded-xl border border-admin-200 shadow-sm overflow-hidden">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-admin-100 bg-white/50">
                      <th className="w-10 text-left text-xs font-semibold text-admin-500 uppercase tracking-wider px-4 py-3">#</th>
                      {columns.map((col, i) => (
                        <th key={i} className={`text-left text-xs font-semibold text-admin-500 uppercase tracking-wider px-4 py-3 ${col.className || ''}`}>
                          {col.header}
                        </th>
                      ))}
                      <th className="text-right text-xs font-semibold text-admin-500 uppercase tracking-wider px-4 py-3">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paged.map((row, idx) => (
                      ...
                      >
                        ...
                        {columns.map((col, i) => (
                          <td key={i} className={`px-4 py-3 text-sm ${col.className || ''}`}>
                            <div className={!disableReply && !row.is_read ? 'font-semibold text-admin-900' : 'text-admin-700'}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-1">
                            ...
                            {!disableReply && <button onClick={e => { e.stopPropagation(); setReplyTo(row); }}
                              className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium bg-white text-admin-700 hover:bg-admin-100 transition-all"
                              title="Reply"
                            >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-admin-100 bg-white/50">
              <div className="flex items-center gap-2 text-xs text-admin-500">
                <span>Rows per page:</span>
                <select value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }}
                  className="h-7 px-2 rounded-md border border-admin-200 bg-white text-xs text-admin-700 focus:outline-none focus:ring-2 focus:ring-admin-500/20"
                >
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
                  className="p-1.5 rounded-md text-admin-400 hover:text-admin-600 hover:bg-admin-100 transition-all disabled:opacity-30"
                ><FiChevronLeft className="w-4 h-4" /></button>
                <span className="text-xs text-admin-500 px-2">{page} / {totalPages || 1}</span>
        ...
        {selected && (
          <div className="bg-white rounded-xl border border-admin-200 shadow-lg overflow-hidden sticky top-4">
            <div className="flex items-center justify-between px-5 py-4 border-b border-admin-100">
              <h3 className="text-sm font-semibold text-admin-900">Details</h3>
              <button onClick={() => setSelected(null)} className="p-1 text-admin-400 hover:text-admin-600 rounded-md hover:bg-admin-100 transition-all">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-t border-admin-100 bg-white/50">
              <div className="flex items-center gap-2">
                ...
                {!disableReply && <button onClick={() => setReplyTo(selected)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-white text-admin-700 hover:bg-admin-100 transition-all"
                >
