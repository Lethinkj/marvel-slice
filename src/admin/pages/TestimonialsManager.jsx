  ...
  return (
    <span className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <FiStar key={i} className={`w-3.5 h-3.5 ${i < count ? 'fill-brand-orange text-brand-orange' : 'text-gray-300'}`} />
      ))}
    </span>
  );
}
...
export default function TestimonialsManager() {
  ...
  async function deleteItem(id) {
    if (!id) return;
    if (!(await confirm('Delete this testimonial? This cannot be undone.'))) return;
  ...
  const columns = [
    ...
    { header: 'Name', accessor: 'name', cell: (row) => <span className="font-semibold text-black">{row.name}</span> },
    ...
    >
      <div className="bg-white shadow-sm border border-admin-200 overflow-hidden">
        {items.length > 0 ? (
          ...
          <EmptyState icon={FiStar} title="No testimonials"
