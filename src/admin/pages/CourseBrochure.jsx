...
const categories = ['Software Learning', 'Competitive Exam', 'Services', 'Training', 'Career'];
  ...
  useEffect(() => {
    async function load() {
      const [coursesRes, navRes] = await Promise.all([
        supabase.from('courses').select('*, highlights(*), overview_faqs(*), course_fees(*), projects(*), certifications(*), course_tabs(*)').order('id', { ascending: false }),
    ...
    <PageShell backTo="/admin"
      ...
      {!loading && (
        <div className="text-center py-16 text-neutral-400">
          ...
          <p className="text-sm font-medium">Click "Download" to generate a brochure PDF</p>
          <p className="text-xs mt-1">Select courses by category or individually to include in the PDF</p>
        </div>
      )}
    </PageShell>
  );
...
function GenerateDialog({ courses, navItems, getCourseCategory, onClose }) {
  ...
  function toggleCategory(cat) {
    setSelectedCategories(prev => {
      ...
      if (next.has(cat)) { next.delete(cat); return next; }
      next.add(cat);
      return next;
    });
  ...
  function getSelectedCoursesList() {
    if (selectedCategories.size > 0) return courses.filter(c => selectedCategories.has(getCourseCategory(c)));
    if (selectedCourses.size > 0) return courses.filter(c => selectedCourses.has(c.id));
    return [];
  }
  ...
  async function generate() {
    ...
    if (selectedCategories.size > 0) label = Array.from(selectedCategories).join('-').toLowerCase().replace(/\s+/g, '-');
    else if (selectedCourses.size === courses.length) label = 'all-courses';
    else if (selectedCourses.size > 0) label = `selected-${selectedCourses.size}-courses`;
    pdf.save(`${label}.pdf`);
    setGenerating(false);
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-lg border border-admin-200 w-full max-w-lg max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <button onClick={selectAll} className="text-xs text-admin-600 font-medium hover:underline">
            ...
            <div className="max-h-52 overflow-y-auto border border-admin-100 rounded-lg divide-y divide-admin-50">
              {(() => {
                ...
                const paginated = display.slice((safePage - 1) * 5, safePage * 5);
                return paginated.length === 0 ? (
                  ...
                  <button key={course.id} onClick={() => toggleCourse(course.id)}
                    ...
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${selectedCourses.has(course.id) ? 'bg-admin-600 border-admin-600' : 'border-admin-300'}`}>
              ...
              return (
                <div className="flex items-center justify-center gap-1 pt-2">
                  <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page <= 1}
                    className="px-2 py-1 text-xs font-medium text-admin-500 hover:text-admin-900 disabled:opacity-30 disabled:cursor-not-allowed">Prev</button>
                  {Array.from({ length: pages }, (_, i) => i + 1).map(p => (
                    <button key={p} onClick={() => setPage(p)}
                      className={`w-7 h-7 rounded text-xs font-medium transition-colors ${p === page ? 'bg-admin-600 text-white' : 'text-admin-500 hover:bg-admin-100'}`}>{p}</button>
                  ))}
                  <button onClick={() => setPage(p => Math.min(pages, p + 1))} disabled={page >= pages}
                    className="px-2 py-1 text-xs font-medium text-admin-500 hover:text-admin-900 disabled:opacity-30 disabled:cursor-not-allowed">Next</button>
                </div>
              );
            })()}
          </div>
        ...
        <div className="px-6 py-4 border-t border-admin-200 flex items-center justify-between shrink-0">
          ...
          <AdminButton onClick={generate} disabled={!hasSelection || generating} variant="primary" size="md">
