  ...
  return (
    <PageShell title={isNew ? 'New Post' : 'Edit Post'} backTo="/admin/blog">
      ...
      <form ref={formRef} onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            {slugStatus === 'taken' && (
              <p className="text-xs text-destructive-500 mt-1">
                ...
                <button type="button" onClick={() => { setForm({ ...form, slug: slugSuggestion }); setSlugStatus('checking'); }} className="text-admin-600 hover:underline font-medium">{slugSuggestion}</button>
              </p>
            )}
        ...
        <ImageUploader value={form.image_url} onChange={(v) => setForm({ ...form, image_url: v })} label="Featured Image URL" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr_auto_auto] gap-4 items-end pt-2 border-t border-admin-100">
          ...
          <div>
            ...
            {allTags.length === 0 ? (
              <p className="text-sm text-neutral-400">No tags. Create in <Link to="/admin/tags" className="text-admin-600 hover:underline">Tags Manager</Link>.</p>
            ) : (
              <div className="relative">
                <div
                ...
                >
                  {selectedTags.length === 0 && <span className="text-neutral-400 text-sm">Select tags...</span>}
                  {selectedTags.map(id => {
                    ...
                    return tag ? (
                      <span key={id} className="inline-flex items-center gap-1 px-2 py-0.5 bg-admin-100 text-admin-700 rounded-md text-xs font-medium">
                        {tag.name}
                        <button type="button" onClick={(e) => { e.stopPropagation(); setSelectedTags(selectedTags.filter(t => t !== id)); }} className="hover:text-admin-900">
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 max-h-48 overflow-y-auto admin-scrollbar bg-white border border-admin-200 rounded-lg shadow-lg z-20 py-1">
                      {allTags.map(tag => (
                        <label key={tag.id} className="flex items-center gap-2 px-3 py-2 hover:bg-neutral-50 cursor-pointer">
                          <input type="checkbox" checked={selectedTags.includes(tag.id)} onChange={() => { setSelectedTags(prev => prev.includes(tag.id) ? prev.filter(t => t !== tag.id) : [...prev, tag.id]); }} className="rounded border-admin-300 text-admin-600 focus:ring-admin-500" />
          ...
          {/* Published toggle */}
          <div>
            <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Published</label>
            <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-admin-200 cursor-pointer hover:bg-admin-100 transition-colors h-[38px]">
              <div className={`relative w-9 h-5 rounded-full transition-colors shrink-0 ${form.is_published ? 'bg-admin-600' : 'bg-admin-300'}`}>
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${form.is_published ? 'translate-x-4' : ''}`} />
                <input type="checkbox" checked={form.is_published} onChange={(e) => setForm({ ...form, is_published: e.target.checked })} className="sr-only" />
              </div>
              <span className="text-sm font-medium text-black whitespace-nowrap">{form.is_published ? 'Live' : 'Draft'}</span>
            </label>
          </div>

          {/* Featured checkbox */}
          <div>
            <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Featured</label>
            <label className="flex items-center gap-2 p-2 bg-white rounded-lg border border-admin-200 cursor-pointer hover:bg-admin-100 transition-colors h-[38px]">
              <input type="checkbox" checked={form.is_featured} onChange={(e) => setForm({ ...form, is_featured: e.target.checked })} className="w-4 h-4 rounded text-admin-600 focus:ring-admin-500/20" />
              <span className="text-sm font-medium text-black whitespace-nowrap">Featured</span>
            </label>
          </div>
        </div>
