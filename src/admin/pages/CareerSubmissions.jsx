...
function MultiSelect({ label, options, selected, onChange, className }) {
  ...
  return (
    <div ref={ref} className={'relative ' + (className || '')}>
      <label className="block text-xs font-semibold text-neutral-600 mb-1 uppercase tracking-wider">{label}</label>
      <button onClick={() => setOpen(!open)}
        className="flex items-center justify-between w-full min-w-[180px] px-3 py-2 border border-neutral-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-accent-500">
        <span className={someSelected ? 'text-neutral-900' : 'text-neutral-400'}>{displayText}</span>
        <FiChevronDown className={'w-4 h-4 text-neutral-400 transition-transform ' + (open ? 'rotate-180' : '')} />
      </button>
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 z-20 bg-white border border-neutral-200 rounded-lg shadow-lg max-h-56 overflow-y-auto">
          <label className="flex items-center gap-2 px-3 py-2 hover:bg-neutral-50 cursor-pointer border-b border-neutral-100 text-sm font-medium text-neutral-600">
          ...
          {options.map((opt) => (
            <label key={opt} className="flex items-center gap-2 px-3 py-2 hover:bg-neutral-50 cursor-pointer text-sm text-neutral-700">
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-neutral-900">Career Submissions</h1>
                  ...
                  {paged.map((s, i) => (
                    <tr key={s.id} className="hover:bg-neutral-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {s.file_url ? (
                          <a href={s.file_url} target="_blank" rel="noopener noreferrer" className="text-accent-600 hover:underline text-xs font-medium">View</a>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-lg border border-neutral-200 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-900 whitespace-pre-wrap">{selected.description || '—'}</p>
              </div>
              {selected.file_url && (
                <div>
                  <label className="block text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-1">Document</label>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-lg border border-neutral-200 w-full max-w-md max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-600 mb-1 uppercase tracking-wider">Category</label>
            <div className="max-h-36 overflow-y-auto border border-neutral-200 rounded-lg divide-y divide-neutral-100">
              <label className="flex items-center gap-2 px-3 py-1.5 hover:bg-neutral-50 cursor-pointer text-sm font-medium text-neutral-600 bg-neutral-50/50">
                <input type="checkbox"
              ...
              {CATEGORIES.map((c) => (
                <label key={c} className="flex items-center gap-2 px-3 py-1.5 hover:bg-neutral-50 cursor-pointer text-sm text-neutral-700">
