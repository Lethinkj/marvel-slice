      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-dark-navy">{navItem ? navItem.label : 'Page Editor'}</h1>
              {pageUrl ? (
                ...
                <p className="text-sm text-amber-600 mt-0.5">Set a path on this nav item for the page to be public.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-dark-navy">Content Sections</h2>
            <Button type="button" onClick={addSection} variant="link-add" size="sm">
          ...
          {sections.length === 0 && (
            <p className="text-sm text-gray-400 italic py-8 text-center border-2 border-dashed border-gray-200 rounded-xl">
          ...
          <div className="space-y-3">
            {sections.map((sec, idx) => (
              <div key={idx} className="border border-gray-200 rounded-xl p-5 hover:border-gray-300 transition-colors">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Section {idx + 1}</span>
                    <select value={sec.section_type} onChange={(e) => updateSection(idx, 'section_type', e.target.value)}
                      className="text-xs border border-gray-300 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-brand-accent bg-white">
                      {sectionTypes.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                    </select>
                  </div>
                  <div className="flex items-center gap-1">
                    <button type="button" onClick={() => moveSection(idx, -1)}
                      className="p-1.5 text-gray-400 hover:text-dark-navy hover:bg-gray-100 rounded-lg transition-colors">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-dark-navy">Linked Courses</h2>
            {courseChanges && <span className="text-xs text-amber-600 font-medium">Unsaved changes</span>}
          </div>
          <p className="text-sm text-gray-500 mb-4">Select courses to display on this page.</p>
          ...
          {allCourses.length === 0 ? (
            <p className="text-sm text-gray-400 italic py-6 text-center border-2 border-dashed border-gray-200 rounded-xl">
                ...
                .map((course) => (
                  <label key={course.id}
                    className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors ${
                      linkedCourseIds.has(course.id) ? 'bg-brand-accent/5 border border-brand-accent/20' : 'hover:bg-gray-50 border border-transparent'
                    }`}>
                    <div className={`relative w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${
                      linkedCourseIds.has(course.id) ? 'bg-brand-accent border-brand-accent' : 'border-gray-300'
                    }`}>
                    ...
                    <Link to={`/admin/courses/${course.id}`} className="text-sm text-dark-navy font-medium hover:text-brand-accent truncate"
