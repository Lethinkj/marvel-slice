...
export default function TestimonialEditor() {
      ...
      if (!isNew) {
        ...
        if (data) {
          setForm({
            ...
            rating: data.rating ?? 5, avatar_url: data.avatar_url || '',
  ...
  if (loading) return <div className="p-8 text-center text-neutral-500">Loading testimonial...</div>;

  return (
    <PageShell backTo="/admin/testimonials" title={isNew ? 'Add Testimonial' : 'Edit Testimonial'}>
      <SaveBar saving={saving} saved={saved} saveError={saveError} label="Testimonial" top />
      <form onSubmit={handleSave}>
        <div className="bg-white border border-gray-300 rounded-xl p-6 space-y-4" style={{ boxShadow: 'rgba(100, 100, 111, 0.2) 0px 7px 29px 0px' }}>
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_1.5fr_1fr] gap-4">
            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Name <span className="text-destructive-500">*</span></label>
            ...
            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Role / Company</label>
              <input name="role" value={form.role} onChange={handleChange} placeholder="e.g. Frontend Developer at TCS"
            ...
            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Rating (1-5)</label>
              <div className="flex items-center gap-3">
                ...
                <span className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <FiStar key={i} className={`w-4 h-4 ${i < (parseInt(form.rating, 10) || 0) ? 'fill-brand-orange text-brand-orange' : 'text-gray-300'}`} />
                  ))}
                </span>
              </div>
            </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-700 mb-1.5 uppercase tracking-wider">Quote <span className="text-destructive-500">*</span></label>
