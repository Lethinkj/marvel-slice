...
export default function BlogManager() {
  ...
  const cardsColumns = [
      ...
      cell: (row) => (
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <Link to={`/admin/blog/${row.id}`} className="font-semibold text-slate-900 hover:text-indigo-600 transition-colors">
              {row.title}
            </Link>
            <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-400">
              ...
              {row.blog_categories && <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full">{row.blog_categories.name}</span>}
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            <div className="w-9 h-5 bg-slate-200 rounded-full peer-checked:bg-indigo-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-4" />
          </label>
    ...
    },
    {
      cell: (row) => (
        <div className="flex items-center gap-2">
          <Link to={`/admin/blog/${row.id}`} className="px-3 py-1.5 text-xs font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-md transition-colors">Edit</Link>
      ...
      {posts.length > 0 && (
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all bg-white"
          />
        </div>
      )}
