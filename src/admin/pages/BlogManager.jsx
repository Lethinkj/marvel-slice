      ...
      cell: (row) => (
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <Link to={`/admin/blog/${row.id}`} className="font-semibold text-admin-900 hover:text-admin-600 transition-colors">
              {row.title}
            </Link>
            <div className="flex items-center gap-3 mt-0.5 text-xs text-admin-400">
              <span className="flex items-center gap-1">
                <FiCalendar className="w-3 h-3" />
                {row.published_at ? new Date(row.published_at).toLocaleDateString() : 'Not published'}
              </span>
              {row.blog_categories && <span className="px-2 py-0.5 bg-white text-admin-600 rounded-full">{row.blog_categories.name}</span>}
    ...
    },
    {
      cell: (row) => (
        <div className="flex items-center gap-2">
          ...
          <FiChevronRight className="w-4 h-4 text-admin-300 transition-colors shrink-0" />
        </div>
      ),
    },
  ];
      ...
      {posts.length > 0 && (
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-admin-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-admin-500/20 focus:border-admin-500 transition-all bg-white"
          />
        </div>
      )}
