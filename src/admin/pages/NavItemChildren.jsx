            ...
            <div className="col-span-2">
              ...
              <div className="relative">
                <button type="button" onClick={() => setCoursePickerOpen(!coursePickerOpen)}
                  ...
                  <span className={form.course_ids.size > 0 ? 'text-admin-900' : 'text-admin-400'}>
                ...
                {coursePickerOpen && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-admin-200 rounded-lg shadow-lg z-50 max-h-56 overflow-y-auto">
                    {courses.map(c => {
                      ...
                      return (
                        <label key={c.id} className={`flex items-center gap-2 px-3 py-2 text-sm cursor-pointer transition-colors ${linkedToOther ? 'text-admin-300 cursor-not-allowed' : 'hover:bg-white text-admin-700'}`}>
