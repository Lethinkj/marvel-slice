...
export default function NavItemManager() {
  ...
  return (
    <PageShell title="Nav Item Manager">
      ...
      {sections.length === 0 ? (
        <div className="text-center py-16 text-slate-400 text-sm border rounded-xl bg-white">No nav items found.</div>
      ) : (
                    ...
                    return (
                      <div key={item.id} className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <span className="text-sm font-medium text-slate-900 flex-1 truncate">{item.label}</span>
                          {item.path && (
                            <span className="text-[11px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full truncate max-w-[120px] hidden sm:inline">{item.path}</span>
                          )}
                            ...
                            <div className="flex gap-1">
                              {itemCourses.map(c => (
                                <Link key={c.id} to={`/admin/courses/${c.id}`}
                                  className="text-[11px] text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full truncate max-w-[120px] hover:bg-indigo-100 transition-colors flex items-center gap-1">
                          ...
                          <Link to={`/admin/nav-menu/children/${item.id}`}
                            className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-slate-500 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors shrink-0">
                          ...
                          <div className="relative shrink-0" ref={courseDropdown === item.id ? dropdownRef : null}>
                            <button onClick={() => setCourseDropdown(courseDropdown === item.id ? null : item.id)}
                              className={`px-2.5 py-1.5 text-xs font-medium rounded-lg transition-colors flex items-center gap-1 ${courseDropdown === item.id ? 'bg-indigo-100 text-indigo-700' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}>
                              <FiBookOpen className="w-3.5 h-3.5" /> Linked Courses
                            </button>
                            {courseDropdown === item.id && (
                              <div className="absolute top-full right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 min-w-[220px] max-h-[280px] flex flex-col">
                                <div className="overflow-y-auto">
                                  {courses.length === 0 ? (
                                    <p className="px-3 py-4 text-xs text-slate-400 text-center">No courses available.</p>
                                  ) : (
                                    courses.map(c => {
                                      const checked = itemCourses.some(lc => lc.id === c.id);
                                      return (
                                        <label key={c.id}
                                          className="flex items-center gap-2.5 px-3 py-2 hover:bg-slate-50 cursor-pointer text-sm border-b border-slate-100 last:border-b-0">
                                          <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors ${checked ? 'bg-indigo-600 border-indigo-600' : 'border-slate-200'}`}>
                        ...
                        {children.length > 0 && (
                          <div className="ml-5 mt-2 space-y-1">
                            {children.map(child => {
                              ...
                              return (
                                <div key={child.id} className="flex items-center gap-2 text-xs text-slate-500 pl-3 border-l-2 border-indigo-200 py-1">
                                  <FiChevronDown className="w-3 h-3 text-slate-300 -rotate-90" />
                                  <span className="text-slate-700">{child.label}</span>
                                  {child.path && <span className="text-slate-400">{child.path}</span>}
                                  {childCourses.length > 0 && (
                                    <div className="flex gap-1 ml-auto">
                                      {childCourses.map(c => (
                                        <Link key={c.id} to={`/admin/courses/${c.id}`} className="text-indigo-600 hover:underline">
                                          {c.title}
                                        </Link>
                                      ))}
                                    </div>
